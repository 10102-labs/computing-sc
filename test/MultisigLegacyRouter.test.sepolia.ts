...
import { MultisigLegacy__factory, MultisigLegacyRouter, MultisigLegacyRouter__factory } from "../typechain-types";
import { MultisigLegacy, MultisigLegacyStruct } from "./../typechain-types/contracts/MultisigLegacy";
import fs from "fs";
...
describe("MultisigRouter", function () {
  /* config */
  const SEPOLIA_RPC_URL = process.env.SEPOLIA_RPC_URL;
  const CHAIN_ID = process.env.SEPOLIA_CHAIN_ID;
  const provider = new ethers.JsonRpcProvider(SEPOLIA_RPC_URL);

  const MULTISIG_LEGACY_ROUTER = process.env.MULTISIG_LEGACY_ROUTER as string;
  ...
  /* Get router contract */
  async function getMultisigLegacyRouter() {
    const MultisigRouterFactory: MultisigLegacyRouter__factory = await ethers.getContractFactory("MultisigLegacyRouter");
    const MultisigRouterContract: MultisigLegacyRouter = MultisigRouterFactory.attach(MULTISIG_LEGACY_ROUTER) as MultisigLegacyRouter;
    return MultisigRouterContract;
  }

  /* Get legacy contract */
  async function getMultisigLegacy(legacyAddress: string) {
    const MultisigLegacyFactory: MultisigLegacy__factory = await ethers.getContractFactory("MultisigLegacy");
    const MultisigLegacyContract: MultisigLegacy = MultisigLegacyFactory.attach(legacyAddress) as MultisigLegacy;
    return MultisigLegacyContract;
  }
  ...
  async function getMetaTransactionData(nameFn: string, arg: Object): Promise<MetaTransactionData> {
    const routerAbiJson = getAbi("../artifacts/contracts/MultisigLegacyRouter.sol/MultisigLegacyRouter.json");
  ...
  /* Struct */
  type MainConfig = MultisigLegacyRouter.LegacyMainConfigStruct;
  type ExtraConfig = MultisigLegacyStruct.LegacyExtraConfigStruct;

  /* Functions */
  async function checkActiveLegacy(legacyId: bigint): Promise<boolean> {
    const MultisigLegacyrouter: MultisigLegacyRouter = await getMultisigLegacyRouter();
    const tx = await MultisigLegacyrouter.checkActiveLegacy(legacyId);
    return tx;
  }
  ...
  async function setBeneficiariesLimit(numBeneficiariesLimit: bigint) {
    ...
    await MultisigLegacyrouter.connect(signer).setBeneficiaryLimit(numBeneficiariesLimit);
  }

  async function createLegacy(safeWallet: string, mainConfig: MainConfig, extraConfig: ExtraConfig, signer: ContractRunner) {
    const MultisigLegacyRouter: MultisigLegacyRouter = await getMultisigLegacyRouter();
    const tx = await MultisigLegacyRouter.connect(signer).createLegacy(safeWallet, mainConfig, extraConfig);
    return tx;
  }
  ...
  async function activeLegacy(legacyId: bigint, signer: ContractRunner) {
    const MultisigLegacyRouter: MultisigLegacyRouter = await getMultisigLegacyRouter();
    const tx = await MultisigLegacyRouter.connect(signer).activeLegacy(legacyId);
    return tx;
  }
  ...
  describe("createLegacy", function () {
    it("Should create legacy successfully", async function () {
      ...
      //State Expect
      const legacyIdExpect: bigint = (await MultisigLegacyRouter._legacyId()) + BigInt(1);
      const legacyAddressExpect: string = await MultisigLegacyRouter.getNextLegacyAddress(signer.address);
      const guardAddressExpect: string = await MultisigLegacyRouter.getNextGuardAddress(signer.address);
      const nonceByUserExpect: bigint = (await MultisigLegacyRouter.nonceByUsers(signer.address)) + BigInt(1);
      ...
      //State After Execute
      const legacyId_: bigint = await MultisigLegacyRouter._legacyId();
      const nonceByUser_ = await MultisigLegacyRouter.nonceByUsers(signer.address);
      const legacyAddress_: string = await MultisigLegacyRouter.legacyAddresses(legacyId_);
      const guardAddress_: string = await MultisigLegacyRouter.guardAddresses(legacyId_);
      const legacy_: MultisigLegacy = await getMultisigLegacy(legacyAddress_);
      ...
      const minRequiredSignatures_: bigint = await legacy_.getMinRequiredSignatures();
    ...
    it("Should revert if length of beneficiaries list difference length of nicknames list ", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "TwoArraysLengthMismatch");
    });
    it("Should revert if not existed beneficiarires", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "EmptyArray");
    });
    it("Should revert if number of beneficiaries > beneficiariesLimit", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "BeneficiaryLimitExceeded");
    });
    it("Should revert if safe wallet existed guard ", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "ExistedGuardInSafeWallet").withArgs(SAFEWALLET_EXIST_GUARD);
    });
  });
  ...
  it("Should revert if signer is not owner of safe wallet ", async function () {
    ...
    //Expect
    expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "SignerIsNotOwnerOfSafeWallet");
  });
  ...
  describe("setLegacyConfig", function () {
    it("Should update legacy config successfully", async function () {
      ...
      const legacyAddress: string = await MultisigLegacyRouter.legacyAddresses(legacyId);
      const legacy: MultisigLegacy = await getMultisigLegacy(legacyAddress);
      ...
      const minRequiredSignatures_: bigint = await legacy.getMinRequiredSignatures();

      //Expect
      expect(activationTrigger_).to.equal(extraConfig.lackOfOutgoingTxRange);
      expect(minRequiredSignatures_).to.equal(extraConfig.minRequiredSignatures);
      expect(beneficiaries_).to.deep.equal(mainConfig.beneficiaries);
      expect(tx).to.emit(MultisigLegacyRouter, "MultisigLegacyConfigUpdated").withArgs(legacyId, mainConfig, extraConfig, timestampExpect);
    });
    it("Should revert if guard of safewallet is invalid", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "GuardSafeWalletInvalid");
    });
    ...
    it("Should revert if module of safewallet is invalid", async function () {
      ...
      //Expect
      expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "ModuleSafeWalletInvalid");
    });
  ...
  describe("setLegacyBeneficiaries", function () {
    it("Should update legacy beneficiaries successfully", async function () {
      ...
      expect(tx)
        .to.emit(MultisigLegacyRouter, "MultisigLegacyBeneficiesUpdated")
  ...
  describe("setActivationTrigger", function () {
    it("Should update legacy activation trigger", async function () {
      ...
      expect(tx).to.emit(MultisigLegacyRouter, "MultisigLegacyCreated").withArgs(legacyId, lackOfOutgoingTxRange, timestampExpect);
    });
    it("Should revert if guard of safewallet is invalid", async function () {
      const MultisigLegacyRouter: MultisigLegacyRouter = await getMultisigLegacyRouter();
      //Input
  ...
  describe("setNameNote", function () {
    it("Should update legacy name note", async function () {
      const MultisigLegacyRouter = await getMultisigLegacyRouter();
      //Input
      const legacyId: bigint = BigInt(1);
      ...
      //Expect
      await expect(tx).to.emit(MultisigLegacyRouter, "MultisigLegacyNameNoteUpdated").withArgs(legacyId, name, note, timestamp);
    });
    ...
    it("Should revert if guard of safewallet is invalid", async function () {
      ...
      //Expect
      await expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "GuardSafeWalletInvalid");
    });
    it("Should revert if module of safewallet is invalid", async function () {
      const MultisigLegacyRouter = await getMultisigLegacyRouter();
      //Input
      ...
      //Expect
      await expect(tx).to.be.revertedWithCustomError(MultisigLegacyRouter, "ModuleSafeWalletInvalid");
    });
  });
  ...
  describe("activeLegacy", function () {
    it("Should active legacy", async function () {
      ...
      const legacyId: bigint = BigInt(1);
      const legacyAddress: string = await MultisigLegacyRouter.legacyAddresses(legacyId);
      const legacy = await getMultisigLegacy(legacyAddress);
      ...
      const thresholdExpect: bigint = await legacy.getMinRequiredSignatures();
