...
async function main() {
    const registry = "0x43a04683dF16C222C57E8f206ad1c694762C1158";
    ...
    const txData = await contract.methods
        .createPlans(
            [ONE_YEAR.toString(), FIVE_YEARS.toString(), LIFETIME.toString()],
            [ONE_YEAR_PRICE, FIVE_YEARS_PRICE, LIFETIME_PRICE],
            ["1 Year", "5 Years", "Lifetime"],
            ["Great for short-term usage, test run, or new user.",
            "Ideal for individuals setting up default long-term legacy plans/",
            "For institutions, foundations, or DAOs with long-term legacy planning needs."
            ],
            ["1-year access\nFull premium functionality\nUpgradable anytime",
            "5-year premium access\nExtendable at any time\nEligible for early token model benefits",
            "5-year premium access\nExtendable at any time\nEligible for early token model benefits"],
